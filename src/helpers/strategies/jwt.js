...
import BaseAuthStrategy from './base-auth'

class JwtStrategy extends BaseAuthStrategy {
  ...
  authenticate(req, callback) {
    ...
    if (!token) {
      return callback.onFailure(new Error('ACCESS_TOKEN_REQUIRED'))
    }
    // Verify the JWT
    JwtStrategy._verifyDefault(token, this._publicKey, this._verifyOpts, function (jwt_err, payload) {
      if (jwt_err) {
